...
"""def read_waypoints(file_path, ego_x, ego_y, ego_yaw_rad, max_distance_ahead, max_distance_left_right):
    ...
    print("Lane -2 Points:")
    for x, y in zip(lane_minus_2_x, lane_minus_2_y):
        print(f"({x:.2f}, {y:.2f})")

    print("\nLane -3 Points:")
    for x, y in zip(lane_minus_3_x, lane_minus_3_y):
        print(f"({x:.2f}, {y:.2f})")        
...
def read_waypoints(file_path, ego_x, ego_y, ego_yaw_rad, max_distance_ahead, max_distance_left_right):
    ...
    print("Lane -2 Points (Sorted by Distance):")
    for x, y, _ in lane_minus_2_points:
        print(f"({x:.2f}, {y:.2f})")

    print("\nLane -3 Points (Sorted by Distance):")
    for x, y, _ in lane_minus_3_points:
        print(f"({x:.2f}, {y:.2f})")
...
def main(data_root, pred_path, vis_path):
    ...
    ego_x, ego_y, ego_yaw_rad = read_odometry("new-2/odometry.csv")
...
if __name__ == '__main__':
    ...
    #data_root  = 'example-data-170347359'
    data_root  = 'new-2'
    #pred_path  = 'CUDA-FastBEV/model/resnet18/result.txt'
    #pred_path  = 'model/resnet18int8/result.txt'
    pred_path  = 'new-2/new-2.txt'
    vis_path   = os.path.join(data_root, "result-final.png")
