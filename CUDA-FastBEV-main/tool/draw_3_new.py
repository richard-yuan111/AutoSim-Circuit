...
def draw_elements(canvas, middle_x, middle_y, left_x, left_y, right_x, right_y, lane_x, lane_y, c_distance):
    ...
    for i in range(0, len(lane_x), 2):
    	if np.hypot(lane_x[i], lane_y[i]) <= c_distance:
        	cv2.circle(canvas, world_to_pixel(lane_x[i], lane_y[i]), 3, (0, 0, 255), -1)
...
def main(data_root, pred_path, vis_path):
    ...
    canvas = draw_elements(canvas, middle_x, middle_y, left_x, left_y, right_x, right_y, lane_minus_2_x, lane_minus_2_y, c_distance)
...
if __name__ == '__main__':
    pred_dir = 'model-lk2/resnet18int8/'  # Directory containing .txt prediction files
    data_root_base = 'example-lk2/'  # Base directory for datasets
    vis_base = 'result-lk2-new/'  # Output directory for visualization results
