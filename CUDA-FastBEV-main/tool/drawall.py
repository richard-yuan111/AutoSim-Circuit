...
#from mmdet3d.structures.bbox_3d.lidar_box3d import LiDARInstance3DBoxes as LB
...
def main(data_root, pred_path, result_dir):
    os.makedirs(result_dir, exist_ok=True)
    pred_files = [f for f in os.listdir(pred_path) if f.endswith('.txt')]

    for pred_file in pred_files:
        set_name = pred_file.replace('.txt', '')
        data_set_path = os.path.join(data_root, set_name)
        pred_file_path = os.path.join(pred_path, pred_file)

        if not os.path.exists(data_set_path):
            print(f"Skipping {pred_file}, corresponding set not found in {data_root}")
            continue

        vis_path = os.path.join(result_dir, f"{set_name}.png")

        print(f"Processing {pred_file} -> {vis_path}")

        info_dict = read_data_file(data_set_path)
        cam_info_dict = info_dict['cams']

        bboxes, scores, cls_arr = read_txt_to_array(pred_file_path)
        ...
        img = np.zeros((900 * 1 + canva_size * scale_factor, 1600 * 3, 3), dtype=np.uint8)
        img[:900, :, :] = np.concatenate(imgs[:3], axis=1)
        #img_back = np.concatenate([imgs[3][:, ::-1, :], imgs[4][:, ::-1, :], imgs[5][:, ::-1, :]], axis=1)
        img = cv2.resize(img, (int(1600 / scale_factor * 3), int(900 / scale_factor * 1 + canva_size)))
...
if __name__ == '__main__':
    data_root = 'example-0216'
    pred_path = 'model-0216/resnet18int8'
    result_dir = 'result4'

    main(data_root, pred_path, result_dir)
