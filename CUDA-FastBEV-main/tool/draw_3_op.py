...
def read_txt_to_array(txt_path):
    ...
    if not lines:
        return np.empty((0, 7)), np.empty((0,)), np.empty((0,)), 0.0

    last_line = lines[-1].strip()

    # Check if the last line is a single value (distance)
    try:
        c_distance = float(last_line)
        lines = lines[:-1]  # Remove the last line from data
    except ValueError:
        c_distance = 0.0

    data_list = [list(map(float, line.strip().split())) for line in lines if line.strip()]
    if not data_list:  # If no valid data lines are present
        return np.empty((0, 7)), np.empty((0,)), np.empty((0,)), c_distance
    ...
    return bboxes, scores, cls_arr, c_distance
...
def draw_elements(canvas, middle_x, middle_y, left_x, left_y, right_x, right_y, lane_x_2, lane_y_2, lane_x_3, lane_y_3, c_distance):
    ...
    for i in range(0, len(lane_x_3), 2):
    	if np.hypot(lane_x_3[i], lane_y_3[i]) <= c_distance:
        	cv2.circle(canvas, world_to_pixel(lane_x_3[i], lane_y_3[i]), 3, (0, 0, 255), -1)
    for i in range(0, len(lane_x_2), 2):
    	if np.hypot(lane_x_2[i], lane_y_2[i]) > (c_distance+2):
        	cv2.circle(canvas, world_to_pixel(lane_x_2[i], lane_y_2[i]), 3, (0, 0, 255), -1)
...
def main(data_root, pred_path, vis_path):
    ...
    #print(cam_info_dict)
    bboxes, scores, cls_arr, c_distance = read_txt_to_array(pred_path)
    ...
    canvas = draw_elements(canvas, middle_x, middle_y, left_x, left_y, right_x, right_y, lane_minus_2_x, lane_minus_2_y, lane_minus_3_x, lane_minus_3_y,  c_distance)
